...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let lmbda = sphfunctions::coeff_static_grav_potential(0.25, gamma, m, r);
    ...
    while t < tf {
        ...
        dt = sphfunctions::time_step_mon(&particles, n, gamma);
    ...
    if let Err(err) = sphfunctions::save_data(&(String::from("./Data/results/toy_star/") + &it.to_string() + &".csv"), &particles){
        println!("{}", err);
        process::exit(1);
    }
    Ok(())
